...
import uk.org.wookey.assembler.WookeyLexer;
...
public abstract class Lexer
{
    ...
    public Token NextToken() throws AsmException {
        ...
        for(;;) {
            t = (token == null) ? __NextToken() : token;
            
            _logger.logInfo(t.detailString());
            token = null;
            previousToken = t;
        ...
        if (t.Type() == Token.EOF) {
            eof = true;
            token = Token_EOF; // return it next time, too.

            if (previousToken == null || previousToken.Type() != Token.EOL) {
            	previousToken = Token_EOL;
